...
#在类外通过对托管属性的直接操作，从而实现类中指定属性的访问、设置、删除。
    ...
    def iso(self,safe_distance=10.0):
        #注意_quarantinepeople数组放的是索引位置
            ...
            for i in range(a):#已有的隔离者检查是否已到隔离时间
                if(self._round - self._timer[self._quarantinepeople[i]]==24 and (self._status[self._quarantinepeople[i]]!=4)):
    ...
    def report(self):
        ...
        self.y1.append(np.count_nonzero((self._people[:, 0]>330) ))
...
if __name__ == '__main__':
    ...
    plt.ion()
    p = People(0, round=300, count=1000, first_infected_count=3, T_infect=20, migrant=10, quarantine=0)
